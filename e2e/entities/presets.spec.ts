...
test.describe('Presets Entity Management', () => {
  ...
  test('should load presets page and validate API response', async ({ page }) => {
    await expect(page.locator('h1, [data-testid="page-title"]')).toBeVisible();

    const response = await apiHelper.waitForApiResponse('/presets');
  ...
  test('should display presets in table format', async ({ page }) => {
    await authHelper.waitForPageLoad();

    const tableFound = await page.locator('table, [data-testid="entity-table"], [role="table"]').isVisible();
    expect(tableFound).toBeTruthy();

    const presetRows = page.locator('tbody tr, [data-testid*="preset-row"]');
    const rowCount = await presetRows.count();
    expect(rowCount).toBeGreaterThanOrEqual(0);
  });

  test('should support presets search', async ({ page }) => {
    await authHelper.waitForPageLoad();
    ...
    if (await searchInput.isVisible({ timeout: 2000 })) {
      ...
      await page.waitForResponse(resp => resp.url().includes('/presets') && resp.status() === 200);

      const searchRequest = apiHelper.getLastApiRequest('/presets');
      if (searchRequest) {
  ...
  test('should view preset details', async ({ page }) => {
    await authHelper.waitForPageLoad();
  ...
  test('should handle preset creation flow', async ({ page }) => {
    await authHelper.waitForPageLoad();
  ...
  test('should validate preset data integrity', async () => {
    const response = await apiHelper.waitForApiResponse('/presets');

    const presets = response.data || (Array.isArray(response) ? response : []);
